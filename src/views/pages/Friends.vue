    ...
    <div class=" w-screen h-screen flex flex-col items-center">
        ...
        <div class=" my-5 w-[500px]">
            ...
            <div class=" border py-3 px-5  mb-2 rounded-lg flex justify-between items-center"
                ...
                <h1>{{ stranger.name }}</h1>
                <button @click="cancelRequestHandler(stranger.id)" v-if="sentOrNot(stranger.id, stranger.email)"
                    class=" px-8 py-2 bg-gray-500 text-white rounded-lg">Cancel Request</button>
                <button v-else @click="addFriendHandler(stranger.id)"
                    class=" px-8 py-2 bg-blue-500 text-white rounded-lg">Add</button>
            </div>
        </div>
    </div>
</template>