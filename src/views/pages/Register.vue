...
const registerHandler = () => {
    axios.post("/register", {
        ...
        .catch(e => {
            responseErrors.value = e.response.data.errors
        });
}
...
<template>
    <div class="h-screen flex flex-col gap-10 items-center justify-center">
        ...
        <form @submit.prevent="registerHandler" class="w-[26rem] mx-auto">
            <div class="mb-5">
                ...
                <span class="error text-red-500" v-if="responseErrors?.name">{{ responseErrors?.name }}</span>
